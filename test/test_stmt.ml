...
let test_csv_renders_totals_without_indentation_or_rules () =
  let jan = p (d 2026 1 1) (d 2026 2 1) in
  let feb = p (d 2026 2 1) (d 2026 3 1) in
  let resolved =
    Stmt.RTotal
  ...
  in
  Alcotest.(check string) "csv total" expected (Stmt.csv resolved)

let test_csv_renders_group_spacing_unknown_labels_and_escaped_values () =
  let jan = d 2026 1 1 in
  let feb = d 2026 2 1 in
  let period = p jan feb in
  let resolved =
  ...
  in
  Alcotest.(check string) "csv group" expected (Stmt.csv resolved)
...
let tests =
  List.map
    ...
    [
      ...
      ( "csv renders totals without indentation or rules",
        test_csv_renders_totals_without_indentation_or_rules );
      ( "csv renders group spacing unknown labels and escaped values",
        test_csv_renders_group_spacing_unknown_labels_and_escaped_values );
    ]
