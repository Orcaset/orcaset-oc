...
let revenue =
      ...
      ( Series.Step
          {
            ...
            queries = [ Self { period = last_period; reduce = Series.reduce_sum } ];
            f =
              (fun values ->
  ...
  Series.unfold ~deps:[]
    (Seq.cons
       (Series.Seed { period = initial_period; f = (fun () -> initial_value) })
...
let () =
  ...
  | [ profit_seq ] ->
      Seq.iter
        (fun cell ->
          let period = Period_cell.cell_period cell in
          let value = Period_cell.eval cell |> snd in
