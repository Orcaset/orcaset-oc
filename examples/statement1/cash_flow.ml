...
let make ~start_date ~freq ~income_stmt_lazy ~capex_pct =
  let net_income_add_back =
    lazy (Lazy.force income_stmt_lazy).Income_statement.net_income |> Lazy.force
  in

  let depreciation_add_back =
    let depr = lazy (Lazy.force income_stmt_lazy).Income_statement.depreciation |> Lazy.force in
    Seq.map (fun acc -> Accrual.map (fun v -> -.v) acc) depr
  in

  let cf_ops = Accrual.sum_seq net_income_add_back depreciation_add_back in
  ...
  let capex =
    let revenue =
      ...
      |> Lazy.force
    in
    Seq.map (fun rev -> Accrual.map (fun v -> v *. capex_pct) rev) revenue
  in
  ...
  let net_cash_change = Accrual.sum_seq (Accrual.sum_seq cf_ops cf_invest) cf_finance in
