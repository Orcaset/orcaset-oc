...
let const_series (type c) ~label (value : float) : c Series.Period.t =
  Series.Period.of_seq ~label
    (Seq.return
...
(* Revenues in GBP *)
let revenue : [ `GBP ] Series.Period.t = const_series ~label:"Revenue (GBP)" 10_000.0

(* Costs in USD *)
let costs : [ `USD ] Series.Period.t = const_series ~label:"Costs (USD)" (-6_000.0)
...
(* let profit = Series.Period.sum ~label:"Bad Profit" revenue costs *)
...
let usd_revenue = Series.Period.convert ~label:"Revenue (USD)" (fun _ r -> r *. 1.34) (lazy revenue)
let profit = Series.Period.sum ~label:"Profit (USD)" [ lazy usd_revenue; lazy costs ]
