...
let revenue_step period =
  ...
  Series.Period.Step
    {
      period;
      queries = [ Self { period = Period.shift offset period; reduce = Series.Period.reduce_sum } ];
      f =
        (fun values ->
...
let revenue =
  ...
  Series.Period.unfold ~deps:[]
    (Seq.cons
       (Series.Period.Seed { period = initial_period; f = (fun () -> 1000.0) })
       (generate_cells initial_period))

(* COGS: 30% of revenue. *)
let cogs = Series.Period.map (fun r -> r *. -0.30) (lazy revenue)

(* Gross Profit: Revenue + COGS. *)
let gross_profit = Series.Period.sum revenue cogs

(* Opex: constant -$200/month. *)
let opex =
  Series.Period.unfold ~deps:[]
    (Seq.map (fun period -> Series.Period.Seed { period; f = (fun () -> -200.0) }) months)

(* Capex: 5% of revenue (negative = cash outflow). *)
let capex = Series.Period.map (fun r -> r *. -0.05) (lazy revenue)
...
let rec depreciation =
  lazy
    (Series.Period.unfold ~deps:[ Point_dep ppe_net ]
       (Seq.map
          (fun period ->
            Series.Period.Step
              {
...
and ppe_net =
  lazy
    (let neg_capex = Series.Period.map (fun x -> -.x) (lazy capex) in
     let dep = Lazy.force depreciation in
     let ppe_change = Series.Period.sum neg_capex dep in
     Series.Point.accum ~start_date ~initial_value:10000.0 (lazy ppe_change))
...
let ebt =
  let gp_opex = Series.Period.sum gross_profit opex in
  Series.Period.sum gp_opex depreciation

(* Tax: 20% of EBT (negative). *)
let tax = Series.Period.map (fun x -> x *. -0.20) (lazy ebt)

(* Net Income: EBT + Tax. *)
let net_income = Series.Period.sum ebt tax

(* --- Cash Flow Statement ----------------------------------------------------- *)

(* Depreciation add back: reverses the non-cash depreciation charge. *)
let dep_add_back = Series.Period.map (fun x -> -.x) (lazy depreciation)

(* CF Operations: Net Income + Depreciation Add Back. *)
let cf_operations = Series.Period.sum net_income dep_add_back

(* CF Financing: constant $0. *)
let cf_financing =
  Series.Period.unfold ~deps:[]
    (Seq.map (fun period -> Series.Period.Seed { period; f = (fun () -> 0.0) }) months)

(* Net Cash Change: CF Operations + Capex + CF Financing. *)
let net_cash_change =
  let ops_plus_capex = Series.Period.sum cf_operations capex in
  Series.Period.sum ops_plus_capex cf_financing

(* --- Balance Sheet ----------------------------------------------------------- *)

(* Cash: starts at $1,000, accumulates net cash change. *)
let cash = Series.Point.accum ~start_date ~initial_value:1000.0 (lazy net_cash_change)

(* Common Stock: constant $5,000. *)
let common_stock = Series.Point.const 5000.0

(* Retained Earnings: starts at $6,000 (initial assets - common stock), accumulates net income. *)
let retained_earnings = Series.Point.accum ~start_date ~initial_value:6000.0 (lazy net_income)
...
let () =
  ...
  in
  let period_seqs = Series.Period.to_seq all_period_series in
  ...
  let cash_cells = Series.Point.query_many end_dates cash in
  let ppe_cells = Series.Point.query_many end_dates ppe_net in
  let common_stock_cells = Series.Point.query_many end_dates common_stock in
  let re_cells = Series.Point.query_many end_dates retained_earnings in
