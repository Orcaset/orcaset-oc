...
module Period : sig
  ...
  val unfold_seq : label:string -> deps:('deps, 'c) deps -> cells:('deps -> 'c cell Seq.t) -> 'c t
  (** [unfold_seq ~label ~deps ~cells] constructs a series from a pre-built cell sequence. Prefer
      this form when the cells are naturally produced by mapping over an existing period sequence
      (e.g. [Seq.map step_fn periods]); use {!unfold} when the cells are generated by threading
      state forward. *)

  val unfold_seq_self : label:string -> cells:(unit -> 'c cell Seq.t) -> 'c t
  (** Convenience wrapper for {!unfold_seq} when the cell builder has no external dependencies. *)

  val unfold :
    label:string ->
    deps:('deps, 'c) deps ->
    init:'s ->
    cells:('deps -> 's -> ('c cell * 's) option) ->
    'c t
  (** [unfold ~label ~deps ~init ~cells] constructs a series by repeatedly applying [cells] to the
      current state, mirroring {!Stdlib.Seq.unfold}. Each call returns [Some (cell, next_state)] to
      emit a cell and advance the state, or [None] to terminate. Use this in preference to
      {!unfold_seq} when cells are generated by threading state forward (e.g. extending a forecast
      from the last historical period). *)

  val unfold_self :
    label:string -> init:'s -> cells:('s -> ('c cell * 's) option) -> 'c t
...
module Point : sig
  ...
  val unfold_seq :
    label:string -> deps:('deps, 'c) deps -> cells:('deps -> 'c cell Seq.t) -> 'c t
  (** [unfold_seq ~label ~deps ~cells] builds a point series from a pre-built sequence of
      period-indexed change cells. Querying the resulting series at a date returns the cumulative
      clipped total through that date. Inside the cell builder, {!Query.self} references the
      constructing point series itself with these as-of semantics. Prefer this form when the cells
      are naturally produced by mapping over an existing period sequence; use {!unfold} when the
      cells are generated by threading state forward. *)
  ...
  val unfold :
    ...
    cells:('deps -> 's -> ('c cell * 's) option) ->
    'c t
  (** [unfold ~label ~deps ~init ~cells] builds a point series by repeatedly applying [cells] to
      the current state, mirroring {!Stdlib.Seq.unfold}. Each call returns [Some (cell, next_state)]
      to emit a cell and advance the state, or [None] to terminate. Accumulation/clipping semantics
      for {!Query.self} are the same as {!unfold_seq}. Use this in preference to {!unfold_seq} when
      cells are generated by threading state forward. *)
